...
public class StudentView {
	...
	public void tblupdt(int id) {
		try {
			...
			model.setRowCount(0);
			int i = 0; // Initialize the row index
			while (res.next()) {
				model.addRow(new Object[0]);
				model.setValueAt(res.getString("dt"), i, 0);
				model.setValueAt(res.getString("status"), i, 1);
				i++; // Increment the row index
			}
			// Debug: Print the table model data
			for (int row = 0; row < model.getRowCount(); row++) {
				System.out.println("TableModel - Row " + row + ": Date: " + model.getValueAt(row, 0) + ", Status: " + model.getValueAt(row, 1));
			}
	...
	public int[] stat(int id) throws SQLException {
		...
		//x[3] = (x[1]*100)/x[0];
		 // Check if x[0] is zero to avoid division by zero
		if (x[0] != 0) 
		{
			x[3] = (x[1] * 100) / x[0];
		} 
		...
		{
			x[3] = 0;// Set percentage to 0 when there are no classes
		}
