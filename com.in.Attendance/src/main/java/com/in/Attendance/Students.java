...
public class Students {
	...
	/*public int getid() throws SQLException {
		Statement stm = con.createStatement();
		ResultSet rst = stm.executeQuery("SELECT COUNT(*) FROM students");
		if (rst.next()) {
			int rowCount = rst.getInt(1);
			if (rowCount == 0) {
				// If no elements in the students table, set id to 1
				return 1;
			} else {
				// If there are elements, fetch the next available id
				rst = stm.executeQuery("SELECT MAX(id) FROM user");
				if (rst.next()) {
					return rst.getInt("MAX(id)") + 1;
				} else {
					return 1;
