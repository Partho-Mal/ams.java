...
public class Admin{
   ...
   //gpt
	public int getid() throws SQLException {
		...
		ResultSet rst = stm.executeQuery("SELECT MAX(id) from user");
		if (rst.next()) {
			String maxIdString = rst.getString("MAX(id)");
			if (!maxIdString.isEmpty()) {
				return Integer.parseInt(maxIdString) + 1;
			}
		}
		return 1; // Default value if no records are found or maxIdString is empty
	}
