...
public class Login{
	...
	public void loginView() {
		...
		//extra
		 // Create a loading panel with a progress bar
		 loadingPanel = new JPanel();
		 loadingPanel.setBounds(0, 0, 1000, 600);
		 loadingPanel.setLayout(new BorderLayout());
		 progressBar = new JProgressBar();
		 progressBar.setIndeterminate(true); // An indeterminate progress bar will keep moving indefinitely
		 progressBar.setPreferredSize(new Dimension(200, 20));
		 loadingPanel.add(progressBar, BorderLayout.CENTER);
 
		 // Initially, the loading panel should be invisible
		...
		login.addActionListener(new ActionListener() {
			...
			@Override
            public void actionPerformed(ActionEvent e) {
                // Show the loading panel while processing
                showLoadingPanel();

                // Perform the login check in a background thread
                SwingWorker<Integer, Void> worker = new SwingWorker<Integer, Void>() {
                    @Override
                    protected Integer doInBackground() throws Exception {
                        return dbCheck(username.getText(), password.getText());
                    }
                    ...
                    protected void done() {
                        try {
                            ...
                            // Hide the loading panel when done
                            hideLoadingPanel();
                        }
                    }
                };
